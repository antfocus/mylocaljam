...
export async function GET(request) {
  ...
  try {
    const { data: hydrated, error } = await supabase
      .from('events')
      .select('*, venues(name, address, color, latitude, longitude, venue_type, tags, photo_url, default_start_time), artists(name, bio, image_url, genres, vibes, is_tribute), event_templates(template_name, bio, image_url, category, start_time, genres)')
      .in('id', collected);
