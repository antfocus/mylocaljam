      ...
      <main style={{
        ...
        <section style={{ marginBottom: '24px' }}>
          ...
          {/* Venue — orange pin link to Google Maps. Outfit weight 600 so it
              feels like part of the headline group, not metadata. Tickets
              indicator lives in the date strip below (informational only —
              users tap Venue or Map in the action row to actually buy). */}
          {venue && (
            <a
              href={mapsUrl}
              target="_blank"
          ...
          {(dateBlock.day || timeStr) && (
            <div style={{
              ...
              {/* Tickets indicator — last segment of the date strip when
                  the event is ticketed. Informational only (no click
                  target — admin pattern is to drive purchase via the
                  Venue / Map action buttons below). Cover string wins
                  if set; otherwise generic "TICKETS" label appears
                  whenever the venue is_ticketed_venue=true OR a ticket
              ...
              {(() => {
                ...
                return (
                  <>
                    {(dateBlock.day || dateBlock.month || timeStr) && (
                      <span style={{ color: t.textDim }}>·</span>
                    )}
                    <span style={{
                      color: t.accent,
                      fontWeight: 700,
                      // $ amounts retain their case; the generic "Tickets"
                      // label rides the strip's existing uppercase rule.
