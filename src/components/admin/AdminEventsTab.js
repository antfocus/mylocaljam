            ...
            {filtered.map((ev) => {
              ...
              <div key={ev.id} className="rounded-xl border" style={{
                ...
                <div style={{
                  ...
                  <button
                    className="p-1.5 rounded hover:text-yellow-400"
                    style={{ color: ev.is_featured ? '#FBBF24' : undefined }}
                    onClick={() => toggleFeatured(ev)}
                    title={ev.is_featured ? 'Remove from Spotlight' : 'Feature in Spotlight'}
                  >
                    {ev.is_featured ? Icons.starFilled : Icons.star}
                  </button>
