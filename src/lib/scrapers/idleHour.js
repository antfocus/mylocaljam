/**
 * Idle Hour Restaurant & Bar scraper
 * Site: https://www.ihpointpleasant.com/
 *
 * Wix site with an embedded Google Calendar on the homepage.
 ...
 *
 * Live music every Friday & Saturday (+ occasional Thursday/special events).
 *
 * If it breaks:
 *   1. Go to https://www.ihpointpleasant.com/
...
const CALENDAR_ID = '8f5d0389c430a2be6bc4445bcb064a60609a1e8abe3e176347e9646c215c0df5@group.calendar.google.com';
const ICAL_URL = `https://calendar.google.com/calendar/ical/${encodeURIComponent(CALENDAR_ID)}/public/basic.ics`;
const VENUE = 'Idle Hour';
const VENUE_URL = 'https://www.ihpointpleasant.com/';
...
function parseIcal(icalText) {
  ...
  for (const line of lines) {
    ...
    if (line === 'END:VEVENT') {
      ...
      const externalId = `idlehour-${eventDateStr}-${uidClean}`;
...
export async function scrapeIdleHour() {
  try {
    ...
    console.log(`[IdleHour] Found ${events.length} upcoming events`);
    return { events, error: null };

  } catch (err) {
    console.error('[IdleHour] Scraper error:', err.message);
