/**
 * Sun Harbor Seafood and Grill scraper
 * Events page: https://www.sunharborseafoodandgrill.com/events
 *
 * Squarespace site — uses the built-in JSON API (?format=json) on the
 * /events collection, which returns structured event data.
 *
 * If it breaks:
 *   1. Go to https://www.sunharborseafoodandgrill.com/events
 ...
 *   4. Try https://www.sunharborseafoodandgrill.com/{collection}?format=json
 *   5. Update COLLECTION below
 */

const BASE_URL = 'https://www.sunharborseafoodandgrill.com';
const COLLECTION = 'events';
const VENUE = 'Sun Harbor Seafood and Grill';
const VENUE_URL = 'https://www.sunharborseafoodandgrill.com/events';

export async function scrapeSunHarbor() {
  try {
    ...
    for (const item of items) {
      ...
      const externalId = `sunharbor-${dateStr}-${idClean}`;
    ...
    console.log(`[SunHarbor] Found ${events.length} upcoming events`);
    return { events, error: null };

  } catch (err) {
    console.error('[SunHarbor] Scraper error:', err.message);
