/**
 * Doyle's Pour House scraper
 * Site: https://doylespourhouse.com/event-calendar/
 *
 * WordPress + Astra/Elementor + The Events Calendar (Tribe) plugin theming,
 * but the actual event data is published via an EMBEDDED PUBLIC GOOGLE
 * CALENDAR iframe — Tribe's REST endpoint at /wp-json/tribe/events/v1/events
 * is empty (the staff/owner blocks on the page are Elementor-rendered, not
 * Tribe events). Same iCal-feed pattern as Idle Hour, McCanns, Jacks on the
 * Tracks, and St. Stephen's Green.
 *
 * Calendar ID: b0umkkkth5lktq8o1v6nji7dcg@group.calendar.google.com
 *   (extracted from the iframe `src=` param on /event-calendar/)
 *
 * If it breaks:
 *   1. Visit https://doylespourhouse.com/event-calendar/ in a browser.
 *   2. Inspect the embedded iframe — its src should still start with
 *      `https://calendar.google.com/calendar/embed?src=`. The value of the
 *      `src=` query param is the calendar ID.
 *   3. Update CALENDAR_ID below if the venue swapped calendars.
 *
 * Address: 210 W Main St, Tuckerton, NJ 08087
 */

const CALENDAR_ID = 'b0umkkkth5lktq8o1v6nji7dcg@group.calendar.google.com';
const ICAL_URL = `https://calendar.google.com/calendar/ical/${encodeURIComponent(CALENDAR_ID)}/public/basic.ics`;
const VENUE = "Doyle's Pour House";
const VENUE_URL = 'https://doylespourhouse.com/event-calendar/';
...
function parseIcal(icalText) {
  ...
  // iCal supports line continuation: any line beginning with whitespace is
  // a continuation of the previous line. Unfold before splitting.
  ...
  for (const line of lines) {
    ...
    if (line === 'END:VEVENT') {
      ...
      // External ID includes the date so a recurring event with a shared
      // UID across instances doesn't collide on the unique constraint.
      const uid = current.uid || `${title}-${current.dtstart}`;
      const uidClean = uid.replace(/[^a-zA-Z0-9]/g, '').slice(0, 40);
      const externalId = `doylespourhouse-${eventDateStr}-${uidClean}`;
...
export async function scrapeDoylesPourHouse() {
  try {
    ...
    console.log(`[DoylesPourHouse] Found ${events.length} upcoming events`);
    return { events, error: null };

  } catch (err) {
    console.error('[DoylesPourHouse] Scraper error:', err.message);
