...
- **Event Series Phase 1 prod-verified.** Three-test pass completed April 22 — Test A (unticked, no linkage), Test B (ticked, new series creation), Test C (ticked, existing series reuse via find-or-create dedup). All three passed; no regressions found. Phase 1 is now fully validated in prod. Phase 2 (#34) and Phase 3 (#35) still active and queued. See `HANDOVER.md` — Session April 22, 2026 for the full test record.
- **Venue aliases system (task #36).** The venue autocomplete in the approval modal (`AdminSubmissionsTab.js`) does exact-string matching only. Scraper and OCR output frequently produces venue names that don't match the canonical seed row — e.g. `"EvenTide Grille, Navesink Marina"` from OCR vs `"Eventide Grille"` in the `venues` table. The mismatch forces the admin to manually trim the string, or produces accidental duplicate venue rows when they click "+ Create New Venue". Fix path: mirror the artist-aliases pattern — either add an `aliases text[]` column to `venues`, or a separate `venue_aliases` table keyed to `venue_id`. Update the admin lookup query (and the underlying API endpoint) to match canonical `name` ILIKE OR any alias. Consider case-insensitive + substring fallback as a final tier.
- **Series name typeahead (task #37).** The "Event / Series Name" field in the approval modal is a plain text input with no autocomplete. Admins must manually retype the exact series name for the find-or-create dedup in `POST /api/admin/queue` to reuse an existing parent row. Slug normalization is case- and punctuation-tolerant, but not word-tolerant — extra qualifiers (e.g. `"TEST 2"` vs `"TEST"`) produce different slugs and create duplicate parent rows. Fix path: debounced typeahead against `event_series.name` (ILIKE) that surfaces matching rows and on-select pre-populates both the name field and the `series_category` dropdown. Could ship standalone or fold into Phase 3 `AdminSeriesTab` (#35).
